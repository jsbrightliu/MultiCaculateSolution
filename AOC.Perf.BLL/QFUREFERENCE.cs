...
namespace AOC.Perf.BLL
{
	...
	{
		private readonly AOC.Perf.DAL.QFUREFERENCE dal=new AOC.Perf.DAL.QFUREFERENCE();
		public QFUREFERENCE()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public AOC.Perf.Model.QFUREFERENCE GetModelByCache(int QFUREFCODE)
		{
			
			string CacheKey = "QFUREFERENCEModel-" + QFUREFCODE;
			...
			return (AOC.Perf.Model.QFUREFERENCE)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<AOC.Perf.Model.QFUREFERENCE> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<AOC.Perf.Model.QFUREFERENCE> DataTableToList(DataTable dt)
		{
			List<AOC.Perf.Model.QFUREFERENCE> modelList = new List<AOC.Perf.Model.QFUREFERENCE>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				AOC.Perf.Model.QFUREFERENCE model;
