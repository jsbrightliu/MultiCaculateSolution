...
namespace AOC.Perf.BLL
{
	...
	{
		private readonly AOC.Perf.DAL.STATE dal=new AOC.Perf.DAL.STATE();
		public STATE()
		{}
		#region  BasicMethod
		...
		{
			return dal.DeleteList(AOC.Perf.Common.PageValidate.SafeLongFilter(STATECODElist,0) );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public AOC.Perf.Model.STATE GetModelByCache(int STATECODE)
		{
			
			string CacheKey = "STATEModel-" + STATECODE;
			...
			return (AOC.Perf.Model.STATE)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<AOC.Perf.Model.STATE> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<AOC.Perf.Model.STATE> DataTableToList(DataTable dt)
		{
			List<AOC.Perf.Model.STATE> modelList = new List<AOC.Perf.Model.STATE>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				AOC.Perf.Model.STATE model;
